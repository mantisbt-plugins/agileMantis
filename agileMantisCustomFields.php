...
$cs->sprint_id = $story['sprint'];
$getSprint = $cs->getSprintById();
	...
	<tr '.helper_alternate_class().'>
		...
		<td>';
			if(plugin_config_get('gadiv_storypoint_mode') == 1){
				echo '<input type="text" name="storypoints" value="'.$story['storypoints'].'" '.$readonly.' '.$disable_storypoints.'>';
			} else {
...
if(plugin_config_get('gadiv_tracker_planned_costs')=='1' && $cs->customFieldIsInProject("PlannedWork")==true){
	echo '
		<td style="background-color:#B3B3CC;font-weight:bold;color:#000;">'.lang_get( 'PlannedWork' ).' '.$story['unit'].'</td>
		<td>
...
echo '
	...
	echo '
		...
		<td colspan="3">
			<select name="sprint" '.$sprintDis.'>';?>
				...
				?>
					<option value="<?php echo $row['sname']?>" <?php if($row['sname'] == $story['sprint']){echo 'selected';$selected = true;}?>><?php echo $row['sname']?></option>
				<?php }}?>
				<?php if($cs->getUserStoryStatus($project_id) >= 80 && !empty($story['sprint']) && $selected == false){?>
	...
	if(plugin_config_get('gadiv_presentable')=='1' && $cs->customFieldIsInProject("Presentable")==true){
		if($story['presentable'] == 3){$marked_3 = 'selected';}
		if($story['presentable'] == 1){$marked_1 = 'selected';}
		if($story['presentable'] == 2){$marked_2 = 'selected';}
		if(plugin_config_get('gadiv_release_documentation')=='1' && $cs->customFieldIsInProject("inReleaseDocu")==true && plugin_config_get('gadiv_technical')=='1' && $cs->customFieldIsInProject("Technical")==true){
			$colspan = 0;
		} elseif(plugin_config_get('gadiv_release_documentation')=='1' && $cs->customFieldIsInProject("inReleaseDocu")==true){
			$colspan = 1;
		} elseif(plugin_config_get('gadiv_technical')=='1' && $cs->customFieldIsInProject("Technical")==true){
			$colspan = 1;
		} else {
			$colspan = 5;
		}
	...
	if(plugin_config_get('gadiv_technical')=='1' && $cs->customFieldIsInProject("Technical")==true){
		if(plugin_config_get('gadiv_release_documentation')=='1' && $cs->customFieldIsInProject("inReleaseDocu")==true && plugin_config_get('gadiv_presentable')=='1' && $cs->customFieldIsInProject("Presentable")==true){
			$colspan = 0;
		} elseif(plugin_config_get('gadiv_presentable')=='1' && $cs->customFieldIsInProject("Presentable")==true){
			$colspan = 3;
		} elseif(plugin_config_get('gadiv_release_documentation')=='1' && $cs->customFieldIsInProject("inReleaseDocu")==true){
			$colspan = 1;
		} else {
		...
		echo '
				...
				<td colspan="'.$colspan.'">
					<input type="checkbox" style="width:10px;" name="technical" value="1" '.$checked.'>
				</td>
		';
	}
	if(plugin_config_get('gadiv_release_documentation')=='1' && $cs->customFieldIsInProject("inReleaseDocu")==true){
		if(plugin_config_get('gadiv_technical')=='1' && $cs->customFieldIsInProject("Technical")==true && plugin_config_get('gadiv_presentable')=='1' && $cs->customFieldIsInProject("Presentable")==true){
			$colspan = 0;
		} elseif(plugin_config_get('gadiv_presentable')=='1' && $cs->customFieldIsInProject("Presentable")==true){
			$colspan = 3;
		} elseif(plugin_config_get('gadiv_technical')=='1' && $cs->customFieldIsInProject("Technical")==true){
		...
		echo '
				...
				<td colspan="'.$colspan.'">
					<input type="checkbox" style="width:10px;" name="inReleaseDocu" value="1" '.$checked.'>
				</td>
