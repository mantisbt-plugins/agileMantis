...
<tr style="background-color:<?php echo $uscolor;?>">
	<td>
		...
		echo $row['id']?>" height="16" width="16">
			<img src="images/update.png" alt="Detailinformation zur User Story bearbeiten" 
			height="16" width="16"></a><?php }?>
	</td>
	<td>
	...
	<td width="20">
		...
		<img src="<?php echo AGILEMANTIS_PLUGIN_URL?>images/info-icon.png" alt="<?php 
			echo plugin_lang_get( 'product_backlog_show_info' );?>" onclick="<?php 
	...
	<td><?php echo $row['rankingOrder']?></td>
	<?php }?>
	<?php if( config_get( 'show_project_target_version', null, auth_get_current_user_id() ) == 1 ) {
		...
		include( AGILEMANTIS_PLUGIN_URI.'pages/sprint_backlog_version.php' );
	?>
	<?php }?>
	<td>
		<form action="<?php echo plugin_page("task_page.php")?>&us_id=<?php 
			...
			<input type="submit" name="add_task" value="<?php 
...
if( !empty( $t ) ) {
	foreach( $t AS $key => $value ) {
		...
		$changed = false;
		$status = $agilemantis_agm->agileMantisStatusColorsAndNames( $value['status'] );
		...
		<tr style="<?php echo $style?>">
			...
			<td style="background-color:<?php echo $uscolor;?>;"></td>
			<td style="background-color:<?php echo $uscolor;?>;"></td>
			<td style="background-color:<?php echo $bgcolor;?>;"><?php echo string_display_links($value['name'])?></td>
			...
			<td style="background-color:<?php echo $bgcolor;?>;width:190px;">
				<form action="<?php echo plugin_page("sprint_backlog.php")?>" method="post">
					...
					<input type="hidden" name="task_description" value="<?php echo string_display_links($value['description'])?>">
			...
			<td style="background-color:<?php echo $bgcolor;?>;">
				<form action="<?php echo plugin_page("edit_task.php")?>" method="post">
					...
					<input type="submit" name="edit" value="<?php 
						echo plugin_lang_get( 'button_edit' )?>" <?php 
						echo $disable?> <?php 
						echo $disable_button?> <?php 
						echo $sprint_end_disable?>>
				</form>
				<form action="<?php echo plugin_page("sprint_backlog.php")?>" method="post">
					<input type="hidden" name="sprintName" value="<?php echo string_display_links($s['name'])?>">
					<input type="hidden" name="task_id" value="<?php echo $value['id']?>">
					<input type="hidden" name="us_id" value="<?php echo $row['id']?>">
					<input type="hidden" name="developer_id" value="<?php echo $value['developer_id']?>">
					<input type="hidden" name="task_name" value="<?php echo string_display_links($value['name'])?>">
					<input type="hidden" name="task_description" value="<?php echo string_display_links($value['description'])?>">
					<input type="hidden" name="task_status" value="<?php echo $value['status']?>">
					<input type="hidden" name="rest_capacity" value="<?php echo $value['rest_capacity']?>">
					<input type="hidden" name="planned_capacity" value="<?php echo $value['planned_capacity']?>">
					<input type="hidden" name="performed_capacity" value="<?php echo $value['performed_capacity']?>">
					<input type="submit" name="divide_task" value="<?php 
						echo plugin_lang_get( 'button_assume' )?>" <?php 
				 ...
				 <?php if( $s['status'] == 0 || ($value['developer_id'] == 0 
					...
					<form action="<?php echo plugin_page("sprint_backlog.php")?>" method="post">
						...
						<input type="submit" name="deleteTask" value="<?php 
							echo plugin_lang_get( 'button_remove' )?>" <?php 
							echo $disable?> <?php 
							echo $disable_button?> <?php 
							echo $sprint_end_disable?>>
				...
				<?php } else {?>
					...
					<form action="<?php echo plugin_page("sprint_backlog.php")?>" method="post">
						...
						<input type="hidden" name="performed_capacity" value="<?php echo $value['performed_capacity']?>">
						<input type="hidden" name="fromSprintBacklog" value="1">
						<input type="submit" name="resolved" value="<?php 
							echo plugin_lang_get( 'button_resolve' )?>" <?php 
							if( $value['status'] >= 4 ) { 
								?>disabled<?php 
							}?><?php 
							echo $sprint_end_disable?>>
					...
					<form action="<?php echo plugin_page("edit_task.php")?>" method="post">
						...
						<input type="hidden" name="action" value="editTask">
						<input type="hidden" name="name" value="<?php echo string_display_links($value['name'])?>">
						<input type="hidden" name="description" value="<?php echo string_display_links($value['description'])?>">
