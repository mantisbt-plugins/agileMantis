...
<a class="version_tooltip" href="javascript: void(0)">
	<?php echo project_get_name($row['project_id'])?> <?php echo $row['target_version']?>
	<span>
		<b><?php echo project_get_name($row['project_id'])?> <?php echo $row['target_version']?></b>
		<br><br>
		...
		<?php echo plugin_lang_get( 'product_backlog_amount' )?>: <?php 
			echo $agilemantis_version->getVersionTracker( 
					$row['project_id'],$row['target_version'], '80,90') 
					+ $agilemantis_version->getVersionTracker(
