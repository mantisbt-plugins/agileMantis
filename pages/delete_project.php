<?php 
	...
	$projectname = $project->getProjectById($_POST['project_id']);
?>
	<br>
	<div align="center">
		<hr width="50%">
			...
			<form action="<?php echo plugin_page('edit_product_backlog.php') ?>" method="post">
				...
				<input type="hidden" name="id" value="<?php echo $_POST['product_backlog_id']?>">
				<input type="hidden" name="project_id" value="<?php echo $_POST['project_id']?>">
				<input type="hidden" name="delete_user_stories" value="<?php echo $_POST['delete_with_warning']?>">
				<input type="submit" name="deleteProject" value="<?php echo plugin_lang_get( 'delete_project_title' )?>" class="button">
