...
<table align="center" class="width100" cellspacing="1">
	<tr>
		<td colspan="5"><b><?php echo $name?> - <?php echo plugin_lang_get( 'sprint_backlog_chose_sprint' )?></b></td>
		<td colspan="2">
			<form action="<?php echo plugin_page($_GET['page'])?>" method="post" style="float:right">
				<?php if($_GET['klickStatus']==1){?>
				<input type="hidden" name="disable_click" value="1">
				<?php }?>
				<input type="hidden" name="do_not_enter_sprint" value="1">
	...
	<tr>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=id&klickStatus=<?php echo $klick?>">Sprint</a></td>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=start&klickStatus=<?php echo $klick?>"><?php echo plugin_lang_get( 'sprint_backlog_begin' ) ?></a></td>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=end&klickStatus=<?php echo $klick?>"><?php echo plugin_lang_get( 'sprint_backlog_end' ) ?></a></td>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=rest&klickStatus=<?php echo $klick?>"><?php echo plugin_lang_get( 'sprint_backlog_rest' ) ?></a></td>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=team&klickStatus=<?php echo $klick?>">Team</a></td>
		<td class="category"><a href="<?php echo plugin_page($_GET['page'])?>&sort_by=pb&klickStatus=<?php echo $klick?>">Product Backlog</a></td>
	...
	<?php
		...
		if($_SESSION['ISMANTISUSER'] && !$_SESSION['ISMANTISADMIN']){
					...
					if($row['team_id'] == $value['team_id']){?>
						<?php if($_GET['page'] == 'daily_scrum_meeting.php'){?>
								...
								<form action="<?php echo plugin_page($_GET['page'])?>" method="post">
									<input type="hidden" name="sprintName" value="<?php echo $row['sname']?>">
									...
									<td><?php echo $sprint->getTeamById($row['team_id']);?></td>
									<td><?php echo $sprint->getProductBacklogByTeam($row['team_id']);?></td>
									<td><input type="submit" name="submit" value="<?php echo plugin_lang_get( 'sprint_backlog_backlog' )?>"></td>
								</form>
							</tr>
							<?php } ?>
						<?php } else {?>
