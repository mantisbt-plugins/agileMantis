		...
		<?php
			...
			if ( !bug_is_readonly( $f_bug_id ) ) {
				...
				if ( $t_can_edit_note || access_has_bug_level( config_get( 'update_bugnote_threshold' ), $f_bug_id ) ) {
					print_button( plugin_page('bugnote_edit_page.php&tid='.$_GET['tid'].'&bugnote_id='.$t_bugnote->id, true), lang_get( 'bugnote_edit_link' ) );
				}
				...
				if ( $t_can_delete_note || access_has_bug_level( config_get( 'delete_bugnote_threshold' ), $f_bug_id ) ) {
					echo " ";
					print_button( plugin_page('bugnote_delete.php&tid='.$_GET['tid'].'&bugnote_id='.$t_bugnote->id, true), lang_get( 'delete_link' ) );
				}
				...
				if ( $t_can_edit_note || ( access_has_bug_level( config_get( 'update_bugnote_threshold' ), $f_bug_id ) &&
					...
					if ( VS_PRIVATE == $t_bugnote->view_state ) {
						echo " ";
						print_button( plugin_page('bugnote_set_view_state.php&tid='.$_GET['tid'].'&private=0&bugnote_id=' . $t_bugnote->id, true), lang_get( 'make_public' ) );
					} else {
						echo " ";
						print_button( plugin_page('bugnote_set_view_state.php&tid='.$_GET['tid'].'&private=1&bugnote_id=' . $t_bugnote->id, true), lang_get( 'make_private' ) );
					}
				}
			}
		?>
