...
# along with agileMantis. If not, see <http://www.gnu.org/licenses/>.
	...
	echo '<us_summary>'.$agilemantis_commonlib->safeCData($userstory[0]['summary']).'</us_summary>';
	echo '<us_status>'.$userstory[0]['status'].'</us_status>';
	echo '<us_description>' . $agilemantis_commonlib->safeCData($userstory[0]['description']) .'</us_description>';
	
	echo '<us_category><![CDATA['.$agilemantis_tasks->getCategoryById($userstory[0]['category_id']).']]></us_category>';
	echo '<us_project_version><![CDATA['.$agilemantis_project->getProjectName($userstory[0]['project_id']) . ' ' . $userstory[0]['target_version'].']]></us_project_version>';
	...
	echo '<version>';
		echo '<us_version_project>'.$agilemantis_commonlib->safeCData($agilemantis_project->getProjectName($userstory[0]['project_id'])).'</us_version_project>';
		echo '<us_version_project_description>'.$agilemantis_commonlib->safeCData($agilemantis_project->getProjectDescription($userstory[0]['project_id'])).'</us_version_project_description>';
		...
		echo '<us_version_trackerGesamt>'.$agilemantis_version->getVersionTracker($userstory[0]['project_id'],$userstory[0]['target_version'], '10,20,30,40,50,60,70,80,90').'</us_version_trackerGesamt>';
		echo '<us_version_trackerOpen>'.$agilemantis_version->getVersionTracker($userstory[0]['project_id'],$userstory[0]['target_version'], '10,20,30,40,50,60,70').'</us_version_trackerOpen>';
		echo '<us_version_userStoriesGesamt>'.$agilemantis_version->getVersionUserStories($userstory[0]['project_id'],$userstory[0]['target_version']).'</us_version_userStoriesGesamt>';
		echo '<us_version_userStoriesOpen>'.$agilemantis_version->getNumberOfUserStories($userstory[0]['project_id'],$userstory[0]['target_version']).'</us_version_userStoriesOpen>';
	...
	if(!empty($addFields)){
		echo '<additionalFields>';
			echo '<field>';
				...
				echo '<field_value>'.$agilemantis_commonlib->safeCData($addFields['name']).'</field_value>';
			...
			echo '<field>';
				...
				echo '<field_value>'.$agilemantis_commonlib->safeCData($addFields['sprint']).'</field_value>';
				echo '<field_datatype>String</field_datatype>';
			echo '</field>';
	...
	$temp_tasks = $agilemantis_sprint->getSprintTasks($id);
	if(!empty($temp_tasks)){
		echo '<tasks>';
		foreach($temp_tasks AS $num => $row){
			...
			echo '<task>';
				...
				echo '<task_name>'.$agilemantis_commonlib->safeCData($row['name']).'</task_name>';
				...
				echo '<task_created>'.$agilemantis_tasks->getUserName($created['user_id']).' / '.date('d.m.Y',$create_date).'</task_created>';
				}
				if($confirmed['user_id'] > 0){
				echo '<task_confirmed>'.$agilemantis_tasks->getUserName($confirmed['user_id']).' / '.date('d.m.Y',$confirm_date).'</task_confirmed>';
				}
				if($resolved['user_id'] > 0){
				echo '<task_resolved>'.$agilemantis_tasks->getUserName($resolved['user_id']).' / '.date('d.m.Y',$resolve_date).'</task_resolved>';
				}
				if($closed['user_id'] > 0){
					echo '<task_closed>'.$agilemantis_tasks->getUserName($closed['user_id']).' / '.date('d.m.Y',$close_date).'</task_closed>';
				}
				if($reopened['user_id'] > 0){
					echo '<task_reopened>'.$agilemantis_tasks->getUserName($reopened['user_id']).' / '.date('d.m.Y',$reopen_date).'</task_reopened>';
				}
			echo '</task>';
		}
		echo '</tasks>';
	...
	if(!empty($notices)){
		...
		foreach($notices AS $num => $row){
			echo '<notice>';
				...
				echo '<note_description>' . $agilemantis_commonlib->safeCData($row['note']) .'</note_description>';
